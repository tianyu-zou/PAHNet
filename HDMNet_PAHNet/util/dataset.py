...
def add_gaussian_noise(img, std=25, noise_ratio=1.0):
    """
    向图像加入高斯噪声

    Args:
        img: np.array，float32格式图像，值域 0~255
        std: 噪声强度
        noise_ratio: 添加噪声的像素比例（0~1）

    Returns:
        加噪后的图像
    """
    noise = np.random.normal(0, std, img.shape).astype(np.float32)
    mask = np.random.rand(*img.shape[:2]) < noise_ratio
    if img.ndim == 3:
...
class SemData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for k in range(self.shot):  
            ...
            # support_image = add_gaussian_noise(support_image, std=50, noise_ratio=1.0)
        ...
        if self.mode == 'train':
            ...
            # return image, label, s_x, s_y, subcls_list, raw_label, image_entropy
            return image, label, label_b, s_x, s_y, subcls_list, raw_label, raw_label_b, image_path, support_image_path
        elif self.mode == 'demo':
