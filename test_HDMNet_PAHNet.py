...
from HDMNet_PAHNet.model import HDMNetPlus

from HDMNet_PAHNet.util import dataset
from HDMNet_PAHNet.util import transform, transform_tri, config
from HDMNet_PAHNet.util.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, get_save_path, \
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='HDMNetPlus')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='HDMNet_PAHNet/config/pascal/pascal_split1_resnet50_manet.yaml', help='config file') # coco/coco_split0_resnet50.yaml
...
def main():
    ...
    for val_id in range(val_num):
        ...
        fb_iou, fb_iou_m, miou, miou_m, miou_b, piou = validate(val_loader, model, val_seed, args.episode)
...
def validate(val_loader, model, val_seed, episode):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            # output_save = output.clone()
            # target_save = ori_label.clone()
            ...
            if args.ori_resize:  # 真值转化为方形
                ...
                # output_save = F.interpolate(output_save, size=ori_label.size()[-2:], mode='bilinear', align_corners=True)
                # output_save = F.interpolate(output_save, size=target.size()[1:], mode='bilinear', align_corners=True)
            ...
            # output_save = output_save.max(1)[1]
            ...
            # std = [0.229, 0.224, 0.225]
            # std = [item * value_scale for item in std]
            # mean = torch.from_numpy(np.array(mean)).view(1, 3, 1, 1).to(input.device)
            # std = torch.from_numpy(np.array(std)).view(1, 3, 1, 1).to(input.device)

            # query_image = input * std + mean
            # query_image = torch.clip(query_image, 0, 255)
            # query_image = query_image[0, ...].detach().cpu().numpy()
            ...
            # query_mask = target_save[0, ...].detach().cpu().numpy()
            # query_mask[query_mask == 255] = 0
            # query_mask = query_mask * 255.

            # support_image = s_input[:, 0, ...] * std + mean
            # support_image = torch.clip(support_image, 0, 255)
            # support_image = support_image[0, ...].detach().cpu().numpy()
            # support_image = support_image.transpose((1, 2, 0))
            # support_image = support_image.astype(np.float32)
            # support_image = cv2.cvtColor(support_image, cv2.COLOR_RGB2BGR)

            # support_mask = s_mask[0, 0, ...].detach().cpu().numpy()
            # support_mask[support_mask == 255] = 0
            # support_mask = support_mask * 255.

            # query_pred = output_save[0, ...].detach().cpu().numpy()
            # query_pred = query_pred * 255.

            # base_path = "samples/{}/fold{}/".format(args.data_set, split)
            # if not os.path.exists(base_path):
            #     os.mkdir(base_path)
            # base_path = "{}{}/".format(base_path, iter_num)
            # if not os.path.exists(base_path):
            #     os.mkdir(base_path)
            # cv2.imwrite("{}{}.png".format(base_path, "image_query"), query_image)
            # cv2.imwrite("{}{}.png".format(base_path, "mask_query"), query_mask)
            # cv2.imwrite("{}{}_{}.png".format(base_path, "pred_query", intersection[1] / (union[1] + + 1e-10)), query_pred)
            # cv2.imwrite("{}{}.png".format(base_path, "image_support"), support_image)
            # cv2.imwrite("{}{}.png".format(base_path, "mask_support"), support_mask)
