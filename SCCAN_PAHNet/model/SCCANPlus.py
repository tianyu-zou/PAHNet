...
from SCCAN_PAHNet.model.ASPP import ASPP
from SCCAN_PAHNet.model.PAHNet_transformer import SwinTransformer
from SCCAN_PAHNet.model.loss import WeightedDiceLoss
from SCCAN_PAHNet.model.PSPNet import OneModel as PSPNet
from einops import rearrange
from SCCAN_PAHNet.model.SSP_matching import SSP_MatchingNet as SSP_matching
...
class OneModel(nn.Module):
    def __init__(self, args):
        ...
        PSPNet_ = PSPNet(args)
        pre_weight = '../initmodel/PSPNet/{}/split{}/{}/best.pth'.format(self.dataset, args.split, "resnet50" if not args.vgg else "vgg")
        print(pre_weight)
        ...
        ssp_weight = "../initmodel/SSPNet/{}/split{}/{}/best.pth".format(self.dataset, args.split, "resnet50" if not args.vgg else "vgg")
    ...
    def forward(self, x, s_x, s_y, y_m, cat_idx=None):
        ...
        query_feat_list, pseudo_masks = self.transformer(query_feat, supp_feat, supp_mask, pro_pred[:, 1, :, :])
        fused_query_feat = []
        ...
        if self.training:
            ...
            for pseudo_mask in pseudo_masks:
                gt_mask = F.interpolate(y_m.unsqueeze(1).float(), size=pseudo_mask.size()[-2:], mode='nearest').squeeze(
                    1)
                aux_loss2 += self.criterion_ce(pseudo_mask, gt_mask.long())
            aux_loss2 /= len(pseudo_masks)
            alpha = 1
