...
import os
# os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8'  # 必须在import torch之前
import torch
...
from SCCAN_PAHNet.model import SCCANPlus

# os.environ['CUDA_VISIBLE_DEVICES'] = "6"
from SCCAN_PAHNet.util import dataset, transform
from SCCAN_PAHNet.util import transform_tri, config
from SCCAN_PAHNet.util.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, \
    get_logger, get_save_path, \
    is_same_model, fix_bn, sum_list, check_makedirs

cv2.ocl.setUseOpenCL(False)
cv2.setNumThreads(0)
# torch.set_deterministic_debug_mode(1)  #终极解决方案
print("#############################")
def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Few-Shot Semantic Segmentation')
    parser.add_argument('--arch', type=str, default='SCCANPlus')  #
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='SCCAN_HPANet_bak/config/pascal/5-shot/pascal_split2_resnet50.yaml',
...
def train(train_loader, val_loader, model, optimizer, optimizer_swin, epoch):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        if args.evaluate and args.SubEpoch_val and ((args.epochs <= 100 and epoch % 1 == 0 and epoch > 0) or (epoch > 100)) and (
                i == round(len(train_loader) / 2)):  # <if> max_epoch<=100 <do> half_epoch Val
            loss_val, FBIoU, mIoU, pIoU = validate(val_loader, model)
            val_num += 1
            # save model for <testing>
    ...
    for e in range(db_epoch):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            if args.ori_resize:
                ...
                # output = F.interpolate(output, size=ori_label.size()[-2:], mode='nearest')
                target = ori_label.long()

            output = F.interpolate(output, size=target.size()[1:], mode='bilinear', align_corners=True)
            # output = F.interpolate(output, size=target.size()[1:], mode='nearest')
